...
do -- Test timer basics.
  ...
  local count = 0
  timer:start(0, 0, function(timer) count = count+1 end)
  assert(timer:is_active())
  loop:run()
  assert(not timer:is_active())
  assert(count == 1)
  timer:start(0, 1e-3, function(timer) count = count+1 end)
  assert(timer:is_active())
  loop:run("once")
  assert(timer:is_active())
  loop:run("nowait")
  ...
  assert(count == 3)
  -- interruption
  timer:start(0, 1e-3, function(timer) timer:stop() end)
  loop:run()
  timer:start(0, 1e-3, function(timer) timer:get_loop():stop() end)
  loop:run()
  timer:start(0, 1e-3, function(timer) timer:close() end)
  loop:run()
end
